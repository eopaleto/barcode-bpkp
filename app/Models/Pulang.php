...
class Pulang extends Model
{
    ...
    {
        static::created(function ($pulang) {
            $pegawai = $pulang->pegawai;
            if (!$pegawai) return;

            $fotoKoper = $pulang->foto_koper ?? [];
            ...
            $canvas->text("No Urut: {$pulang->id}", 150, 30, function ($font) {
                $font->size(14);
                $font->color('#000000');
                $font->align('left');
            });
            ...
            $canvas->text("Barcode untuk Pulang", 150, 125, function ($font) {
                $font->size(12);
                $font->color('#FF0000');
                $font->align('left');
            });
